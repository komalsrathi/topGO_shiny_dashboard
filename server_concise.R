...
# increase file upload size to 30 MB
# set global options for DT::datatable function
options(shiny.maxRequestSize = 30*1024^2,
        DT.options = list(dom = 'RMDCT<"clear">lfrtip',
...
shinyServer(function(input, output, session){
  ...
  ########################### Limma Output  ########################### 
  # when a project is selected, the corresponding limma file is grabbed 
  ...
  # the limma output for selected project is displayed in a new tab
  ...
  # switch to tab1 i.e. 'Limma Output' as soon as a project is selected 
  observe({
    s <- input$projects_rows_selected
    if(length(s)){
  ...
  # the user has the option to select fold change and pvalue cutoff
  # by default fc is 0 and adj.pvalue is 0.05
  # subset data based on fold change & up/down/differentially regulated genes
  ...
  # switch to tab2 on clicking the select button to display filtered data
  observe({
    if(input$select>0){
  ...
  ########################### compute GOdata object & display results - topGO  ########################### 
  # get GOdata object 
  datasetInput4 <- reactive({
    limma <- datasetInput() # get entire limma output i.e. expressed genes
    fg <- datasetInput2() # get foregound i.e. filtered data
    
    validate(
      need(fg$fc, "Check Input...!!")
    )
    
    # select background genes to test in topGO
    if (input$cutoff=='pos') {
      limma <- limma[which(limma$fc>0),] # get all upregulated
    } else if (input$cutoff=='neg') {
      limma <- limma[which(limma$fc<0),] # get all downregulated
    } else if(input$cutoff=='both') {
      limma <- limma                     # get all expressed
    } 
    
    # make a gene universe of adj.pvalues and probe IDs
    all_genes <- limma$adj.P.Val 
    names(all_genes) <- limma$ID 
    
    # make a list of interesting genes to test for enrichment
    ...
    # function that returns list of interesting genes
...
#     }
  ...
  # get results for all selected tests
  datasetInput3 <- reactive({
    validate(
      need(input$cbxgrp1, "Select one or more tests")
    )
    
    GOdata <- datasetInput4() # get the GOdata object computed above
    ...
    test <- as.numeric(input$cbxgrp1) # list of selected tests
    
    # get results for all selected tests for all the GOTerms
  ...
  ########################### compute and display topGO graph  ########################### 
  # update dropdown named 'Algorithms:' with selected tests for topGO graph generation
  observe({
    if(input$topgo > 0)
    {
  ...
  # this is for topGO graph generation for selected test in the dropdown 'Algorithms:'
  ...
  ########################### get limma output for genes in selected GO Terms  ########################### 
  # get GO associated genes 
  datasetInput6 <- reactive({
    limma <- datasetInput() # get limma output
    d <- datasetInput3() # results for selected tests
    s <- input$results_rows_selected # all the GO terms selected
    goid <- d[s, , drop = FALSE]
    ...
    dd <- merge(dd,d[,1:2],by.x='.id',by.y='GO.ID') # display GO term ID along with Limma output for a gene
    
    dd <- dd[,c(1,2,ncol(dd))] 
    dd <- merge(limma, dd, by.x='ID', by.y='Chip.ID') 
    
    # limma data for all genes in selected GO terms
    dd 
  })
  ...
  ########################### get eset and make Heatmap of genes in selected GO terms  ########################### 
  # get eset and convert to dataframe
  datasetInputHeatmap <- reactive({
    
    validate(
      need(input$inputdataset, "Input list of projects")
    )
  ...
  # make dotplot of gene expression for selected gene
  ...
  # output expression plot/dotplot
  ...
  ########################### Heatmap of genes in selected GO Terms  ########################### 
  ...
  ########################### PCA plot for expressed genes  ########################### 
  # PCA plot
  # make pca plot
  pca_plot <- reactive({
    expr <- datasetInputHeatmap()
    vst.var = sort(apply(expr,1,var),decreasing = T)
    s <- as.numeric(input$obs) # number of top variant genes
  ...
  ########################### run SPIA on filtered dataset  ########################### 
  # run SPIA
  spia_out <- reactive({
    limma <- datasetInput() # all expressed genes
    fg <- datasetInput2() # filtered dataset on fc and adj.pvalue
  ...
  # output SPIA results in tab9
  output$spiaout <- DT::renderDataTable({
    input$runspia
    withProgress(session = session, message = 'Calculating...',detail = 'This may take a while...',{
      isolate({
        DT::datatable(spia_out(),
                      escape = FALSE,
                      selection = 'none',
                      extensions = c('TableTools','ColVis','Scroller'))
      })
